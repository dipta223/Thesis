...
class CryptoBlock {
    ...
    mineBlock(difficulty) {
        while (
            this.hash.substring(0, difficulty) !== Array(difficulty + 1).join("0")       //Array(3 + 1).join("0") will generate "000" 
        ) {
            this.nonce++;
            this.hash = this.computeHash();
        }
...
class CryptoBlockchain {
    ...
    startGenesisBlock() {
        let cb = new CryptoBlock(Date.now(), 0, "0");
    ...
    getBalanceOfAddress(address) {
        ...
        const block = this.obtainLatestBlock();
        ...
        for (i = 0; i < sendAddrBalance.length; i++) {
            if (sendAddrBalance[i]['add'] === address) {
                sendAddrBalance[i]['bal'] += balance;
                balance = sendAddrBalance[i]['bal'];
                //console.log("Inside getbalanceof : ", sendAddrBalance[i]['add'] + " : " + sendAddrBalance[i]['bal']);
            }
        }
